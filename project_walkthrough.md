...
### 4. Real-World Engineering Challenges & Bug Fixes
Translating theoretical math into a physical Docker swarm exposed fascinating distributed systems challenges:

*   **The Multiprocessing Bottleneck:** Initially, the backend attempted to use a `ProcessPoolExecutor` to parallelize the mathematical OODA loop across CPU cores. However, on macOS (which uses the `spawn` method instead of `fork` for multiprocessing), this caused severe `BrokenProcessPool` crashes and 500 server errors. Furthermore, we discovered that pickling (serializing) massive node objects back-and-forth between processes actually created extreme IPC overhead. **The Fix:** We refactored the backend to execute the mathematical steps synchronously, which dramatically improved speed and eliminated the crashes.
*   **Asynchronous Desynchronization (The "66/75 Nodes" Bug):** In the physical swarm, the central Tracker initially dropped any node that failed to send a heartbeat within 10 seconds. However, nodes randomly sleep between 3-6 seconds in their OODA loops to simulate network latency, and frequently block while establishing real TCP sockets. This combination often pushed cycle times past 10 seconds, causing nodes to erroneously appear "offline" to the Tracker (resulting in only 66 of 75 nodes showing as active). **The Fix:** We increased the Tracker's heartbeat tolerance to 30 seconds and added strict 2.0-second timeouts to all node-side requests, perfectly synchronizing the UI tracking with the asynchronous physical reality.
